...
### The app, onto the iPhone

⚠️ **`npx expo run:ios` does not work on this machine.** Against **Xcode 26.6**
it prints `Unexpected devicectl JSON version output from devicectl`, fails to
match the device by either UDID form, and then **hangs indefinitely with no
child processes** — it looks like a running build and is not one. Diagnose it
with `pgrep -P <expo pid>`: no children means it is stuck, not compiling. Don't
wait it out, and don't retry it with a different `--device` value.

Drive the toolchain directly instead. This is the working path, verified
2026-08-10:

```bash
# 1. Find the device. Two different IDs, and they are NOT interchangeable:
xcrun xctrace list devices      # classic UDID (00008140-…) → xcodebuild
xcrun devicectl list devices    # CoreDevice UUID (82A4E38A-…) → devicectl

# 2. Build (Release: the JS bundle is embedded, so no Metro at runtime)
xcodebuild -workspace ios/ThePod.xcworkspace -scheme ThePod \
  -configuration Release -destination 'id=<CLASSIC-UDID>' \
  -allowProvisioningUpdates build

# 3. Install
xcrun devicectl device install app --device <COREDEVICE-UUID> \
  ~/Library/Developer/Xcode/DerivedData/ThePod-*/Build/Products/Release-iphoneos/ThePod.app
```

Takes roughly 15 minutes cold. Confirm the bundle is embedded before installing
— `ls ThePod.app/main.jsbundle` should show ~4.3MB with a current timestamp. If
it's missing or stale, the phone will run **old JS** off a fresh binary, which
looks exactly like "my changes didn't apply".

`ios/` is gitignored and regenerated by `npx expo prebuild`. It does **not**
need regenerating for a JS-only change — only when a native dependency or an
`app.json` native setting changes. Prebuilding unnecessarily costs a full pod
install and a cold build.

⚠️ **The install expires after 7 days.** Team `RG725XA2U3` is a **free personal
Apple ID**, so `iOS Team Provisioning Profile: com.alchemy13.ThePod` is issued
for one week (checked with
`security cms -D -i ThePod.app/embedded.mobileprovision | plutil -extract ExpirationDate xml1 -o - -`).
When it lapses the icon stays on the home screen and iOS refuses to launch it —
that is expiry, not a crash, and no amount of debugging the app will fix it.
Re-run the two commands above to reset the week. A paid Developer Program
membership ($99/yr) raises this to a year and is the only route to TestFlight or
the App Store; deferred deliberately as of 2026-08-10.

### The firmware, onto the Pi
